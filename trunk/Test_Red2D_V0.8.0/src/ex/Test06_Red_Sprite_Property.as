package ex
{
	...
	public class Test06_Red_Sprite_Property extends Red_Scene
	{
		...
		private var _testBox:Red_Sprite;
		...
		public override function setDesign():void{
			...
			_testBox = new Red_Sprite(200,200,stage.stageWidth/2,stage.stageHeight/2)
			addChild(_testBox)
		...
		{	
			...
			huiSlider_X=new HUISlider(gui,0,50,"x",setValue)
			huiSlider_X.setSliderParams(0,stage.stageWidth,stage.stageWidth/2)
			
			huiSlider_Y=new HUISlider(gui,0,70,"y",setValue)
			huiSlider_Y.setSliderParams(0,stage.stageHeight,stage.stageHeight/2)
				
			huiSlider_Rotation=new HUISlider(gui,0,90,"rotation",setValue)
			huiSlider_Rotation.setSliderParams(0,360,0)
				
			huiSlider_Alpha=new HUISlider(gui,0,110,"alpha",setValue)
			huiSlider_Alpha.setSliderParams(0,1,1)
				
			huiSlider_ScaleX=new HUISlider(gui,0,130,"scaleX",setValue)
			huiSlider_ScaleX.setSliderParams(0,1,1)
				
			huiSlider_scaleY=new HUISlider(gui,0,150,"scaleY",setValue)
			huiSlider_scaleY.setSliderParams(0,1,1)
				
			huiSlider_Width=new HUISlider(gui,0,170,"width",setValue)
			huiSlider_Width.setSliderParams(0,200,200)
				
			huiSlider_Height=new HUISlider(gui,0,190,"height",setValue)
			huiSlider_Height.setSliderParams(0,200,200)
				
			materialTitle = new Label(gui,0,220,"Material Change")
			
			huiSelecter = new ComboBox(gui,0,250,"colorMaterial",["colorMaterial","bitmapMaterial","movieClipMaterial"])
		...
		protected function HD_combo(event:Event):void
		{
			...
			switch(huiSelecter.selectedIndex){
				case 0:
					_testBox.material = new ColorMaterial(0xffffff)
					break
				case 1:
					_testBox.material = new BitmapMaterial(new MapTest1().bitmapData)
					break
				case 2:
					movieClipMaterial = new MovieClipMaterial(new SWC_TestAir)
					movieClipMaterial.useAnimation=true
					_testBox.material = movieClipMaterial					
					break				
			}		
		}
		...
		{
			_testBox[$e.currentTarget.label] = $e.currentTarget.value
