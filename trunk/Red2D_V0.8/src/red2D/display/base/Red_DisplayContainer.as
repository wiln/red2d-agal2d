package red2D.display.base
{
	...
	public class Red_DisplayContainer extends Red_DisplayObject
	{
		...
		private function applyFilter():void{
			...
			var parents:Red_DisplayContainer = this	
			var imsiTargetFilters:Array = _filters.concat()				
			...
			for (var i:int=0; i<20; ++i){
				if(parents.parent !=null){			
					parents = parents.parent as Red_DisplayContainer
					var parentFilters:Array  = parents.filters.concat().filter(checkFragment_uv);
					finalFilters=parentFilters.concat(finalFilters)
					
				}else{
					break
				}
			}	
			targetFilters = finalFilters.concat(imsiTargetFilters)
			//trace(targetFilters)
		}
		
		private function checkFragment_uv(element:*, index:int, arr:Array):Boolean {
			return (element.type != FilterType.FRAGMENT_UV);
		}
