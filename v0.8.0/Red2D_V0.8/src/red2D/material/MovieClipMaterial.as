package red2D.material
{
	...
	public class MovieClipMaterial extends MaterialBase
	{
		 ...
		 * 
		 */		
		public function MovieClipMaterial( $movieClip:MovieClip, $format:String = Context3DTextureFormat.BGRA, $optimizeRenderToTexture:Boolean=false)
		{
			
			// 실제 사이즈 입력
			_inputWidth =$movieClip.width
			_inputHeight =$movieClip.height
		...
		{			
			...
			_bitmapData.draw( _movieClip , _mapRatioMatrix , null , null, null,true)					
		...
		public function set useAnimation(value:Boolean):void
		{
			...
			if(_useAnimation){
				...
				_interval = setInterval(update,100%60)
			}else{
				clearInterval(_interval)
			}			
		}
